...
**ユニバーサルIDドメインに関する推奨事項（SM-DR11）**: マイクロサービスのあらゆるインスタンスのIDは一貫性と一意性を有するべきである。一貫性として，サービスは動作する場所に関わらず同じ名前を持つべきであり，一意性として，システム全体を通してサービス名はそのサービスのみに対応すべきである。これは，場所によって論理サービスが異なるという意味ではない。各サービスに独自のDNS（Domain Name System）名を割り当てるような典型的なDNSの使い方は，この推奨事項を満たしている。システムポリシーを管理可能にするたえ，サービスの一貫した名前（ID）が必要である。
...
**ID変更時に接続を循環させるサービスプロキシに関する推奨事項（SM-DR14）**: サービスプロキシのID証明書を更新するとき，サービスプロキシは既存の接続を効率的に廃棄し，すべての新しい接続を新規の証明書で確立すべきである。証明書はmTLSのハンドシェイク時のみ検証されるため，新しい証明書の発行時に既存の接続を置き換えることは厳密には必要ない。むしろ，これは時間内に攻撃を制限するために重要である。
