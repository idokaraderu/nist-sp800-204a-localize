...
- SM-AR3: サービスメッシュの機能はプロキシ内に実装されている。各プロキシはマイクロサービスインスタンス[^1]の前に配置され，マイクロサービスベースのアプリケーションにまとまったインフラサービスを提供する。このプロキシは「サイドカープロキシ」と呼ばれ，アプリケーションコードから独立した実装・運用が可能である。サイドカープロキシは種々のプラットフォーム（様々なプログラミング言語やアプリケーション開発フレームワーク）にとっての最小公分母なAPIであるネットワークを用いることにより，これらの一貫した制御を可能としている。
