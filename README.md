...
サービスメッシュのサービスプロキシはイングレストラフィック(サービス間通信ではなく，外部からマイクロサービスアプリケーションに入ってくるトラフィック)の制御にも利用できる。
この意味で，これはAPIゲートウェイの機能を実現する。
概念的に，イングレスコントローラは外部クライアントのサイドカープロキシとして見ることができる。
イングレスコントローラ(フロントプロキシとも呼ばれる)は以下の機能を提供する。

- サービスメッシュ内の実際のAPIを保護する全クライアントに共通のAPI
- HTTP/HTTPS(Hypertext Transfer Protocol Secure)などのWebフレンドリーなプロトコルから，RPC/gRPC/REST(Representational State Transfer)などマイクロサービスで使われるプロトコルへの変換
