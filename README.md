...
マイクロサービス型アプリケーションの一部サービスは，パブリックもしくはプライベートのWeb APIやサードパーティのサービス，レガシーアプリケーション，VM・別クラスタ（サービスメッシュが稼働する以外のクラスタ）のような異なる仮想基盤上のアプリケーションにアクセスしなければならない場合がある。
これらリソースへのアクセスに同様のセキュリティ保証を提供するため，以下の推奨事項が提供される。

**外部リソースのアクセス制限に関する推奨事項（SM-DR8）**: メッシュ外の外部リソースやサービスへのアクセスはデフォルトで無効とし，明示的なポリシーにより特定の宛先に絞ったアクセスのみ許可されるべきである。
加えて，これらの外部リソースやサービスは，サービスメッシュ自体のサービスとしてモデル化されるべきである（例えば，サービスメッシュのサービスディスカバリに含めるなど）。

**外部リソースへのセキュアなアクセスに関する推奨事項（SM-DR9）**: サービスメッシュ内のサービスと同等の可用性向上機能（リトライ制御，タイムアウト，サーキットブレーカなど）を外部のリソースやサービスへのアクセスにも提供しなければならない。

**イーグレスプロキシに関する推奨事項（SM-DR10）**: （スタンドアロンの）イーグレスプロキシも，サービスプロキシやイングレスプロキシと同様にトラフィックルーティングルールを設定する機能を有するべきである。
デプロイされた場合，外部リソースやサービスへのアクセスは，これらのイーグレスプロキシによって中継されるべきである。
イーグレスプロキシは，アクセスポリシーと可用性ポリシー（SM-DR8）が実装されるべきである。
これは，従来のネットワーク指向セキュリティモデルと共に使用する場合に有効である。
例えば，インターネットへの外向きトラフィックがネットワーク内の特定のIPからのみ許可されている場合，イーグレスプロキシをそのIPアドレスで動作させ，メッシュ内の様々なサービスのトラフィックをプロキシさせるよう設定できる。

### 4.4 IDとアクセス管理の設定

マイクロサービス型アプリケーションの主な通信相手は，クライアント，マイクロサービス，外部サービスの3種類ある。
いかなるペア（クライアント－マイクロサービス間，マイクロサービス同士，マイクロサービスー外部サービス間）の通信イベントの間，両エンティティには異なるIDを持たせ，相互認証を行う必要がある。
相互TLS（mTLS）はこれを行うための標準的な仕組みであるため，クライアントまたはマイクロサービスが保持する認証証明書には「subject name(訳者注: TLS証明書内のsubjectフィールドにあるCommon Name)」または「subject alternative name（訳者注: 同じく証明書の拡張領域のパラメータ）」フィールドにそのIDを含んでいる必要がある。
このIDは，サーバID（ホストまたはドメインとしても知られる）またはサービスID（通常はService Account ID）のいずれかである。
証明書のデプロイに関する推奨事項は以下の通りである。

**ユニバーサルIDドメインに関する推奨事項（SM-DR11）**: マイクロサービスのあらゆるインスタンスのIDは一貫性と一意性を持つべきである。
一貫性としてサービスは動作する場所によらず同じ名前を持つべきであり，一意性としてシステム全体を通してサービス名はそのサービスにのみ対応すべきである。
ロケーション（訳者注: IPやポート番号）によって論理サービスが異なることはない。
サービスごとに独自のDNS（Domain Name System）名を持つような典型的なDNSの使い方は，この推奨事項を満たしている。
サービスの一貫した名前（ID）はシステムポリシーを管理可能にするために必要となる。

**証明書への署名に関する推奨事項（SM-DR12）**: サービスメッシュのコントロールプレーンが持つ証明書管理システムは自己署名証明書の生成機能を無効にすべきである。
本機能は，サービスメッシュ内におけるその他の全てのID証明書の初期署名証明書を生成するために良く使用される。
代わりに，メッシュのコントロールプレーンで使用される署名証明書は常に，信頼された企業の既存PKI（Public Key Ingrastructure;公開鍵基盤）のルート（訳者注: ルート証明書）に紐づくものとし，サービスメッシュ起動時にコントロールプレーンへとセキュアに提供されるべきである。
これにより，既存PKIを用いたこれらの証明書の管理（失効処理や監査など）がシンプルになる。
更に，ローテーションを簡素化し，きめ細かな失効を可能にするために，異なるドメインに対して別々の中間署名証明書を生成することが推奨される。

**ID証明書のローテーションに関する推奨事項（SM-DR13）**: マイクロサービスのID証明書の有効期間は，インフラ内で管理可能な範囲で短くすべきである（可能なら数時間オーダー）。
攻撃者は資格情報の有効期限が切れるまでしか資格情報を利用したサービスのなりすましができず，資格情報の再盗用は攻撃の難易度を向上させることから，攻撃の制限に役立つ。

**ID変更時に接続を循環させるサービスプロキシに関する推奨事項（SM-DR14）**: サービスプロキシのID証明書を更新するとき，サービスプロキシは既存の接続を効率的に廃棄し，全ての新規接続を新しい証明書で確立すべきである。
証明書はmTLSのハンドシェイク時のみ検証されるため，新しい証明書の発行時に既存の接続を置き換えることは厳密には必要ない。
むしろ，これは攻撃を一定時間内に制限させるために重要である。

**非署名型ID証明書に関する推奨事項（SM-DR15）**: マイクロサービスのIDに使用される証明書は，署名証明書であってはならない。

**証明書発行前のワークロード認証に関する推奨事項（SM-DR16）**: サービスメッシュのコントロールプレーンが持つ証明書管理システムは，サービスインスタンスにID証明書を発行する前にサービスインスタンスを認証すべきである。
多くのシステムでは，システムのオーケストレーションエンジンに対してインスタンスを証明し，他のローカル証明（例 HSM; Hardware Security Moduleから取得したシークレットなど）を使用することで実現できる。
同様に，サービスメッシュのコントロールプレーンが持つ証明書管理システムの署名証明書をプロビジョニングする際も注意すべきである。
この署名証明書はサービスメッシュのコントロールプレーンによってのみ取得可能で，かつ何らかの形式で証明が行われた後にのみ取得可能とされるべきである。

**セキュアなネーミングサービスに関する推奨事項（SM-DR17）**: mTLSで用いる証明書がサーバIDを伝達する場合，サービスメッシュはサーバIDとマイクロサービス名（セキュアなディスカバリサービスやDNSによって提供されるもの）とを対応付けるセキュアなネーミングサービスを提供すべきである。
この要件はサーバがマイクロサービスから認可されていることを保証し，ネットワークのハイジャックを防ぐために必要となる。

mTLSで用いる証明書がサービスIDを伝達する場合は，追加のセキュアなネーミングサービスは不要である。
これは同様に，マイクロサービスが別のネットワークドメイン（別クラスタや別クラウド）に移植された際も，新環境に合わせたIDや関連アクセス制御ポリシーの再定義が不要にする。

サービスIDに基づきマイクロサービスの証明書を設定すると，通信する2つのサービスはセキュアな通信チャネルを確立できるようになるが，そのロケーションで通信を許可するか否かは指定されない。
これを指定するには，マイクロサービスノードごとに許可された受信・送信トラフィックのポリシーを定義する機能が必要となる（訳者注:認証と認可は別という話だと思われる。ノードがVMや物理ホストを表している可能性もあるが，3章序論を見る限り，マイクロサービスノードはインスタンス（Pod相当）を指していると思われる）。

**きめ細かいIDに関する推奨事項（SM-DR18）**: マイクロサービスごとに独自のIDを持ち，そのマイクロサービスのすべてのインスタンスは，実行時に同じIDを提示すべきである。
これにより，与えられたNamespaceにおいて，マイクロサービスのレベルでのアクセスポリシーが可能となる。
一般的なマイクロサービスは標準だとサービスごとではなくNamespaceごとのIDを発行しており，特に指定がない限り，Namespace内の全てのサービスが実行時に同じIDを提示してしまう。このため，本機能が必要となる。
