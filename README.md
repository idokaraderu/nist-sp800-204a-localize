...
### 4.7 CORS（Cross-Origin Resource Sharing;オリジン間リソース共有）の設定

**CORS（オリジン間リソース共有）に関する推奨事項（SM-DR26）**: エッジサービス（すなわち，マイクロサービスのエントリーポイント）は，WebUIのクライアント側サービスのような外部サービスと通信するために，しばしばCORS設定が必要になる。
...
クラウドや大規模なエンタープライズ環境におけるマイクロサービスベースのアプリケーションの採用が増加していることから、包括的で一貫性があり、調整された一連のサービス支援機能を提供するインフラストラクチャが求められている。
