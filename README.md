...
マイクロサービス型アプリケーションの一部サービスは，パブリックもしくはプライベートのWeb APIやサードパーティのサービス，レガシーアプリケーション，VMや別クラスタ（サービスメッシュが動作するクラスタとは別のもの）のような異なる仮想化インフラのアプリケーションにアクセスしなければならない場合がある。
