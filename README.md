...
**ユニバーサルIDドメインの推奨事項(SM-DR11)**: マイクロサービスのあらゆるインスタンスのIDは一貫性と一意性を有するべきである。一貫性として，サービスは動作する場所に関わらず同じ名前を持つべきであり，一意性として，システム全体を通してサービス名はそのサービスのみに対応すべきである。これは，場所によって論理サービスが異なるという意味ではない。各サービスに独自のDNS(Domain Name System)名を割り当てるような典型的なDNSの使い方は，この推奨事項を満たしている。システムポリシーを管理可能にするたえ，サービスの一貫した名前（アイデンティティ）が必要である。
