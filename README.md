...
サービスメッシュはサービス間通信を円滑化するための一連のインフラ機能を備えた専用のインフラレイヤであり，その機能にはサービスディスカバリ，ルーティング，内部負荷分散，トラフィック設定，暗号化，認証，認可，メトリクス，および監視がある。
これは，サービスインスタンスがオフラインや繰り返し再配置されることでネットワークトポロジが変化する環境において，ネットワークの挙動やマイクロサービスインスタンスの同定，ポリシーを介した通信フローを宣言的に定義する機能を提供する。
これは、OSI(Open Systems Interconnection)参照モデル(例えばTCP/IP;Transmission Control Protocol/Internet Protocol)のトランスポート層の上部にある抽象化層に位置しており、サービスのセッション層(OSI参照モデルの第5層目)の懸念事項に対処するためのネットワーキングモデルと見なすことができる。
