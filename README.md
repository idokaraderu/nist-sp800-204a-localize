...
**セキュアなネーミングサービスに関する推奨事項（SM-DR17）**: mTLSで用いる証明書がサーバIDを運搬する場合，サービスメッシュはサーバIDをセキュアディスカバリサービスやDNSによって提供されるマイクロサービス名にマッピングするセキュアなネーミングサービスを提供すべきである。
この要件はマイクロサービスにとって認可されたロケーションにサーバがあることを確定させ，ネットワークのハイジャックから保護するために必要である。

mTLSで用いる証明書がサービスIDを運ぶ場合，追加のセキュアなネーミングサービスは不要である。
これはマイクロサービスを異なるネットワークドメイン（異なるクラスタやクラウド）に移植した際も，新しい環境に合わせてIDおよび関連付けられたアクセス制御ポリシーを定義し直さなくて良くなる。

サービスIDに基づくマイクロサービスの証明書を設定することで，通信する2つのサービスはセキュアな通信チャネルを確立できる。しかし，その場での通信が全て許可されているかは特定できない。
